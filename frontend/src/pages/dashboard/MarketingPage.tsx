...
export default function MarketingPage() {
    ...
    const calculate = () => {
        // Локальный расчёт пока без backend — так проще итерации
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <h1 style={{ fontSize: 28, fontWeight: 700 }}>📣 Маркетинг</h1>
                <Button onClick={calculate} style={{ width: 'auto', padding: '10px 24px' }}>
                    Рассчитать метрики
                </Button>
            </div>
...
function computeMetrics(d: DashboardForm): MarketingMetrics {
    ...
    const romi = d.marketingCosts === 0 ? 0 : ((d.revenue - d.marketingCosts) / d.marketingCosts) * 100;
    const roi = d.marketingCosts === 0 ? 0 : ((d.grossProfit - d.marketingCosts) / d.marketingCosts) * 100;
    ...
    return {
        funnel: { ctr, crClickLead, crLeadMql, crMqlSql, crSqlMeeting, crMeetingOffer, crOfferDeal, crTotal },
        costs: { cpc, cpm, cpl, cpql, cpsql, cac, cpo },
        roi: { romi, roi, roas, roasFull, marketingShare },
        ltv: { ltv, ltvCac, payback },
        efficiency: { aov, retention, churn, bounceRate, organicShare, marketShare, samShare },
        unit: { marginPerClient, profitPerClient, breakEven, revenuePerClient, profit },
    };
}